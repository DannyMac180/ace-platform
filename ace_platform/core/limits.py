...
class UsageStatus:
    ...
    remaining_storage_bytes: int | None

    # Status flags
    is_within_limits: bool
    limit_exceeded: str | None  # Which limit was exceeded, if any
    current_managed_inference_requests: int = 0
    current_managed_inference_tokens: int = 0
    current_managed_inference_cost_usd: Decimal = Decimal("0")
...
async def get_user_usage_status(
    ...
    managed_inference_summary = await get_usage_counter_summary(
        db,
        user_id,
        period_start,
        now,
    ...
    return UsageStatus(
        ...
        current_managed_inference_requests=managed_inference_summary.request_count,
        current_managed_inference_tokens=managed_inference_summary.total_tokens,
        current_managed_inference_cost_usd=managed_inference_summary.total_cost_usd,
