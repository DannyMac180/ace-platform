...
- Target the `DannyMac180/ace-platform` GitHub repo when creating PRs.
...
2. Run the local `ace-platform` validation gate before pushing:
...
6. Write/update the PR body explicitly for this repo:
   - Include a short summary of changes.
   - Include a concrete test plan with commands actually run.
   - If PR already exists, refresh body content so it reflects the total PR
     scope (all intended work on the branch), not just the newest commits,
     including newly added work, removed work, or changed approach.
   - Do not reuse stale description text from earlier iterations.
7. Use `--repo DannyMac180/ace-platform` when creating the PR.
...
if [ -z "$pr_state" ]; then
  gh pr create --repo DannyMac180/ace-platform --title "$pr_title"
else
  # Reconsider title on every branch update; edit if scope shifted.
  gh pr edit --title "$pr_title"
fi

# Write/edit PR body with a summary and explicit test plan before publishing.
# Example structure:
# Summary:
# - <what changed>
# - <what changed>
...
- `ruff format` is part of this repo's normal pre-PR checklist, so rerun
  `git status` after validation and include any formatter edits intentionally.
